...
-   To get familiar with the `renv` package.
...
1.  To setup your cloud-based development environment create an account using your email at http://rstd.io/class with code `angry-beaver`. Click on the RstudioWorkbench widget to start your environment.
...
3.  Press render to see your new website! Or render via terminal with `quarto render`. Make sure that your web browser allows popups.

4.  Exercise:

    -   Inspect the yaml file

    -   Use the quarto [website](www.quarto.org) to identify what each part of the file does

## Part 2: `renv` workflow

1.  Install and initialize an `renv` workflow.
...
-   Run `.libPaths()` again. How has your library path changed?
...
-   Make a list of the layers that need to be reproduced in this workflow.
    ...
    -   Where are packages being pulled from? Confirm by typing `options("repos")`.
    -   Does this differ from the repo set in Tools \> Global Options \> Packages? You can modify your package repository by running `options("repos" = c("<REPO-NAME>" = "https://your-repository-url.com"))` in your RStudio console.
...
3.  (**Optional)** Learning how to use all the power of Quarto is beyond the scope of the workshop but you can take some time to play around with your new website. Use the below resources to try out difference templates and options.
...
2.  Your project files are saved locally on the server. We want to version control them by adding them to github. Create a [repository](https://docs.github.com/en/migrations/importing-source-code/using-the-command-line-to-import-source-code/adding-locally-hosted-code-to-github#adding-a-local-repository-to-github-using-git) on github but do not initialize it with a README, license, or gitignore files. This repo will host the files for the site that you will be deploying to Github Pages.

3.  We will need to securely authenticate from our server environment to Github. You can do this via two different authentication mechanisms - SSH or HTTPS. Note that there are two urls created for your repo - one is for https and the other is SSH.
...
3.  Start the ssh-agent with `eval "$(ssh-agent -s)"`. The `ssh-agent` is a key manager for SSH. It holds your keys and certificates in memory, unencrypted, and ready for use by ssh.
...
8.  In your Github account go to Settings \> SSH and GPG Keys, and add your SSH key.
...
1.  Create a .github folder with a publish.yaml file:
