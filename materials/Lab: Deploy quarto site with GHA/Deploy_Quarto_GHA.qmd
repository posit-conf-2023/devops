...
1.  Create a github repository named something like `Personal-site`. Copy the https for your repo.
2.  Open a session in your workbench environment. Give it a name.
3.  To create a new website project within RStudio, use the **New Project** command and select **Quarto Website**. Create a git repository upstream.
4.  Quick start https://quarto.org/docs/websites/
5.  Call `renv::init` to initialize a new project-local environment with a private R library. Once you do this you will see a new file called `renv.lock` has been created in your project directory along with an `renv` folder.
