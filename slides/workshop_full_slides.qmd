...
::: notes
these are the ones that we're going to be covering - definitely not going to be a complete overview of the entire ecosystem - but in my opinion the most important tool types to get you started quickly. We will also be discussing this in the context of ds and how these types of tools can fit in a data workflow - and hopefully improve it.
:::
...
| Dev                                                                  | Test                           | Prod                           |
+======================================================================+================================+================================+
| exploratory analysis and experiments                                 | as similar to prod as possible | separate from dev and test     |
|                                                                      |                                |                                |
| often just your local desktop                                        | unit & integration testing     | created using code             |
|                                                                      |                                |                                |
| "sandbox" with data that's as close to real as possible | data validation                | code promotion process + tests |
|                                                                      |                                |                                |
| access to R/Python packages                                          |                                | completely automatic           |
+----------------------------------------------------------------------+--------------------------------+--------------------------------+
...
:::{.notes}
You'll be doing renv in your workbench environment both in your console for r code and in the terminal for git commands
you'll also need to log in to your github account where you'll be creating an upstream repository to link with your local code.
...
::: notes
prod-identical - help you build isolated environments and tested and debug code;  docker, kubernetes, other container services

IaC - quickly re-provision your entire environment with code (chef, puppet, dockr, terraform, ansible) instead of fixing it, just tear it down and restart it

tools that automate the process of building and deploying your code once it goes from your dev env to test and prod

You also have support tools - that are used across a lot of these tools - I want to discuss one of them here -
YAML
:::
...
-   configures files across many execution environments including Docker, virtual machines, K8s, helm, IaaS, etc
...
EmpRecord:  
  ...
  emp02:
    ...
    skills: 
      ...
      - Sales
```
-   whitespace indentation denotes structure
...
-   Docker compose [yaml file](https://github.com/rstudio/rstudio-docker-products/blob/dev/docker-compose.ym) to spin up all 3 Posit Pro products
...
-   Incorporate continuous integration process
-   Work with your version control system

:::{.notes}
- we'll be discussing GHA Actions - which is a CI tool that is handy because it uses the same platform as your versioned repositories

:::

## Quarto Publish GHA Flow
- show regular quarto publish with arrows
- show github actions flow to ghp and connect
...
```         
go through yaml file for the lab
```
...
- create a gh branch
- publish to github
- create a gha with a yaml file
- create an api key for connect
- update yaml file
- push everything
